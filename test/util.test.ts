...
import { normalizePermille, precision, tc2s, tokenizeNote, toMidi } from '../src/util';
...
describe('normalizePermille', () => {
  it('normalizes permille', async () => {
    ...
    expect(normalizePermille(500)).toEqual(0.5);
...
describe('tc2s', () => {
  it('works', async () => {
    expect(tc2s(1200)).toEqual(2);
    expect(tc2s(0)).toEqual(1);
    expect(precision(tc2s(-7973), 2)).toEqual(0.01);
  });
});
